...
export const TOKENS_BULK = (block: number | undefined, tokens: string[]) => {
  ...
  const queryString = `
    query tokens {
      tokens(where: {id_in: ${tokenString}}, orderBy: totalValueLockedUSD, orderDirection: desc) {
        id
        symbol
        name
        derivedETH
...
export function useFetchedTokenDatas(tokenAddresses: string[]): {
  ...
  const formatted = tokenAddresses.reduce((accum: { [address: string]: TokenData }, address) => {
    ...
    let volumeUSD = 0
    const volumeUSDChange = 0
    current ? (volumeUSD = parseFloat(current.tokenDayData.dailyVolumeUSD)) : undefined
    const volumeUSDWeek =
      current && week
    ...
    const tvlUSDChange = getPercentChange(current?.totalValueLockedUSD, current?.tokenDayData.totalLiquidityUSD)
    ...
    const priceUSDOneDay = current ? current.tokenDayData.priceUSD : 0
    ...
    const txCount = current ? parseFloat(current.txCount) - parseFloat('0') : 0
    const feesUSD =
      current && oneDay
        ? parseFloat(current.feesUSD) - parseFloat('0') // changed
        : current
