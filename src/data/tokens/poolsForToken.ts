...
export const POOLS_FOR_TOKEN = gql`
  query topPools($address: ID!) {
    asToken0: pairs(first: 200, orderBy: volumeUSD, orderDirection: desc, where: { token0: $address }) {
      id
    }
    asToken1: pairs(first: 200, orderBy: volumeUSD, orderDirection: desc, where: { token1: $address }) {
      id
    }
  }
`
