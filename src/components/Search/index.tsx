...
const OptionButton = styled.div<{ enabled: boolean }>`
  ...
  align-items: center;
  background-image: ${({ theme, enabled }) =>
    enabled
      ? 'linear-gradient(rgba(255, 0, 0, 0) 0%, rgba(140, 112, 211, 1) 85%, rgba(186, 178, 207, 1) 100%)'
      : 'transparent'};
...
const Search = ({ ...rest }: React.HTMLAttributes<HTMLDivElement>) => {
  ...
  return (
    <Hotkeys keyName="command+/" onKeyDown={handleDown}>
      <Container>
        <Wrapper {...rest} className="cards" data-augmented-ui="border bl-clip br-clip tl-clip tr-clip">
          <StyledInput
            type="text"
            value={value}
            onChange={(e) => {
