...
const DataRow = ({ transaction, color }: { transaction: Transaction; color?: string }) => {
  ...
  const inputTokenSymbol = transaction.amountToken0 < 0 ? transaction.token1Symbol : transaction.token0Symbol
  ...
  return (
    <ResponsiveGrid>
      <ExternalLink href={getEtherscanLink(1, transaction.hash, 'transaction', activeNetwork)}>
        <Label color={color ?? theme.violet1} fontWeight={400}>
      ...
      <Label end={1} fontWeight={400}>
        <ExternalLink
          ...
          style={{ color: color ?? theme.violet1 }}
        >
...
export default function TransactionTable({
  ...
  if (!Array.isArray(transactions)) transactions = transactions.data
  // theming
  const theme = useTheme()
  // for sorting
  ...
  return (
    <Wrapper className="cards" data-augmented-ui="border bl-clip br-clip tl-clip tr-clip">
      <AutoColumn gap="16px">
        <ResponsiveGrid>
          <RowFixed>
            <SortText
